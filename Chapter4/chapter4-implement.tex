...
The implementation and training experiments were conducted using cloud-based GPU environments to efficiently handle large-scale speech processing and model fine-tuning tasks.

The primary development and training environment utilized Kaggle Notebooks with dual NVIDIA Tesla T4 GPUs, leveraging Kaggle’s free 30-hour GPU allocation for long-duration training sessions. Additional experiments and validation runs were performed using Google Colab with NVIDIA Tesla T4 GPUs.
...
\item nlpaug for augmentation
\end{itemize}

The Whisper-small model was fine-tuned using mixed augmented and synthetic dysarthric speech datasets. Training sessions collectively exceeded 14 hours across multiple runs. Audio preprocessing and feature extraction were performed at a sampling rate of 16 kHz using Whisper-compatible log-Mel spectrogram representations.

\section{Baseline Evaluation}
The initial step involves evaluating the performance of the pre-trained Whisper-small model on the TORGO dataset. The evaluation metric used is Word Error Rate (WER), which quantifies transcription accuracy.
...
The Whisper-small model is fine-tuned using a parameter-efficient approach known as Low-Rank Adaptation (LoRA). This method allows efficient training by updating only a small subset of parameters.
...
\item Baseline Evaluation using Whisper-small
...
After fine-tuning and integrating the post-processing layer, the model is evaluated again on the TORGO dataset using WER as the primary metric. The improved model achieves a WER of 16.12\%, demonstrating a substantial improvement over the baseline.
